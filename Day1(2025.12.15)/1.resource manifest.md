...
Readme를 제외한 파일들 유효성검사를 한다.
실제 run은 아니고 dry run임.

#### 외부에서 cluster서버 접속

openshift 는 http/https만 가능하긴 하지만 router로 접속이 가능함.
k8s에서는 ingress controler와 비슷. 
https => termination:edge가 적혀있음.
- router사용 명령어
    oc get route

라벨로 앱 찾기 : oc get all -l  app=exoplanets
해당 라벨을 가진 리소스 삭제 : oc delete -l app=exoplanets

oc get route -l app=exoplanets
